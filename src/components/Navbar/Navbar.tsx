    ...
    return (
        <nav className={`flex flex-row w-full transition-all duration-300 ease-in-out sticky left-0 top-0 text-white z-20 h-15 ${scrolled ? 'bg-[#2f3147]' : 'bg-gradient-to-b from-[#23263a] to-transparent'}`}>
            ...
            <div className="w-full h-full m-2 relative">
                ...
                {showResults && (
                    <div className="absolute left-0 top-12 w-full bg-[#23263a] rounded shadow-lg z-50 max-h-96 overflow-y-auto">
                        <div className="p-3 border-b border-gray-700 text-sm text-gray-300 font-semibold">Danh sách phim</div>
                        {filteredFilms.length === 0 ? (
                            ...
                            filteredFilms.map(film => (
                                ...
                                >
                                    <img src={film.image} alt={film.title} className="w-12 h-16 object-cover rounded" />
                                    <div>
                                        <div className="font-bold text-white">{film.title}</div>
                                        <div className="text-xs text-gray-400">{film.title}</div>
                                        <div className="text-xs text-gray-400">{film.year} • {film.rating}</div>
                                    </div>
                                </Link>
                            ))
                        )}
            ...
            <ul className="items-cente justify-center pl-5 space-x-8 hidden lg:flex flex-auto flex-row w-full">
                ...
                <DropDown path="/" items={[
                    { label: <label>aaa</label>, path: "1" }
                ]}>
                    Thể  loại
                </DropDown>
            </ul>
            <div className="px-5 h-full md:hidden flex items-center justify-center"><FaBars /></div>
        </nav>
    );
}
